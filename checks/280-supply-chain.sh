...
id: P-280
name: Supply Chain
description: Detects supply chain risks, missing dependency pinning, unsigned artifacts.
category: code-scan
severity: medium
languages: any
min_tier: free
...
frameworks: PCI-DSS:4.0:6.3.2, SOC2:TSC-2017:CC9.2, ISO-27001:2022:5.19, ISO-27001:2022:8.28, OWASP-API:2023:API10, CIS-v8:16.4
PRESTON_META
...
snapshot=$(grep -rB3 --include="pom.xml" 'SNAPSHOT' --exclude-dir=".git" --exclude-dir="node_modules" --exclude-dir="vendor" --exclude-dir="dist" --exclude-dir="build" --exclude-dir="target" "$SRC" 2>/dev/null | grep -B3 "SNAPSHOT" | grep "<groupId>\|<artifactId>" | grep -v "test\|Test\|target\|node_modules")
...
version_pinned=$(grep -rn --include="pom.xml" '<version>' --exclude-dir=".git" --exclude-dir="node_modules" --exclude-dir="vendor" --exclude-dir="dist" --exclude-dir="build" --exclude-dir="target" "$SRC" 2>/dev/null | grep -v "test\|Test\|target\|SNAPSHOT\|<!--\|\${" | wc -l | tr -d ' ')
...
trusted_registry=$(grep -rn --include="pom.xml" --include="*.gradle" 'repository\|<url>' --exclude-dir=".git" --exclude-dir="node_modules" --exclude-dir="vendor" --exclude-dir="dist" --exclude-dir="build" --exclude-dir="target" "$SRC" 2>/dev/null | grep -v "test\|Test\|target\|central\|maven\|github\|<!--" | head -3)
