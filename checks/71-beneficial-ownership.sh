...
if [[ ${_go_files:-0} -gt 0 ]]; then
  _go_hits=$(grep -rn --include="*.go" --exclude-dir=.git --exclude-dir=vendor --exclude-dir=node_modules -E "beneficial.*owner|ubo|ultimate.*owner|controlling.*person|ownership.*percent|company.*kyc|entity.*verify|corporate.*verification|business.*verify|ein|tax.*id" "$SRC" 2>/dev/null | grep -vE "_test\.go|/vendor/" || true)
  _go_count=$([[ -n "$_go_hits" ]] && echo "$_go_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_go_count:-0} -gt 0 ]]; then
    record "PASS" "P-71 Beneficial Ownership (Go)" "UBO tracking / entity KYC patterns found in Go code"
  else
    record "WARN" "P-71 Beneficial Ownership (Go)" "No beneficial ownership or entity KYC patterns found in Go files"
  fi
fi
...
if [[ ${_rs_files:-0} -gt 0 ]]; then
  _rs_hits=$(grep -rn --include="*.rs" --exclude-dir=.git --exclude-dir=target -E "beneficial.*owner|ubo|ultimate.*owner|controlling.*person|ownership.*percent|company.*kyc|entity.*verify|corporate.*verification|business.*verify|ein|tax.*id" "$SRC" 2>/dev/null | grep -vE "#\[cfg\(test\)|/tests?/" || true)
  _rs_count=$([[ -n "$_rs_hits" ]] && echo "$_rs_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_rs_count:-0} -gt 0 ]]; then
    record "PASS" "P-71 Beneficial Ownership (Rust)" "UBO tracking / entity KYC patterns found in Rust code"
  else
    record "WARN" "P-71 Beneficial Ownership (Rust)" "No beneficial ownership or entity KYC patterns found in Rust files"
  fi
fi
