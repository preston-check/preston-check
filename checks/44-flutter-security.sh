...
for pubspec in $dart_dirs; do
  ...
  [[ -d "$dart_src" ]] || continue
  secrets=$(grep -rn --include="*.dart" --max-count=5 'apiKey.*=.*"[a-zA-Z0-9]\{20,\}\|secret.*=.*"[a-zA-Z0-9]\{20,\}' "$dart_src" 2>/dev/null | head -3)
  ...
  cert_pin=$(grep -rn --include="*.dart" --include="pubspec.yaml" --max-count=3 "certificate.*pin\|SecurityContext\|badCertificateCallback\|ssl_pinning" "$dart_src" "$(dirname $pubspec)" 2>/dev/null | head -3)
