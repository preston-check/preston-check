...
if [[ ${_go_files:-0} -gt 0 ]]; then
  _go_hits=$(grep -rn --include="*.go" --exclude-dir=.git --exclude-dir=vendor --exclude-dir=node_modules -E "security.*monitor|intrusion.*detect|anomaly.*detect|threat.*detect|auto.*block|auto.*ban|quarantine|auto.*suspend|circuit.*breaker|CircuitBreaker|breaker.*open|fallback|retry.*policy|bulkhead|self.*heal|auto.*recover|auto.*restart|watchdog|firewall|jwt|encrypt|audit|allowlist|blocklist" "$SRC" 2>/dev/null | grep -vE "_test\.go|/vendor/" || true)
  _go_count=$([[ -n "$_go_hits" ]] && echo "$_go_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_go_count:-0} -gt 0 ]]; then
    record "PASS" "P-82 Continuous defense (Go)" "$_go_count pattern(s) found in Go code"
  else
    record "WARN" "P-82 Continuous defense (Go)" "No defense/monitoring/circuit-breaker patterns found in Go files"
  fi
fi
...
if [[ ${_rs_files:-0} -gt 0 ]]; then
  _rs_hits=$(grep -rn --include="*.rs" --exclude-dir=.git --exclude-dir=target -E "security.*monitor|intrusion.*detect|anomaly.*detect|threat.*detect|auto.*block|auto.*ban|quarantine|auto.*suspend|circuit.*breaker|breaker.*open|fallback|retry.*policy|bulkhead|self.*heal|auto.*recover|auto.*restart|watchdog|firewall|jwt|encrypt|audit|allowlist|blocklist" "$SRC" 2>/dev/null | grep -vE "#\[cfg\(test\)|/tests?/" || true)
  _rs_count=$([[ -n "$_rs_hits" ]] && echo "$_rs_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_rs_count:-0} -gt 0 ]]; then
    record "PASS" "P-82 Continuous defense (Rust)" "$_rs_count pattern(s) found in Rust code"
  else
    record "WARN" "P-82 Continuous defense (Rust)" "No defense/monitoring/circuit-breaker patterns found in Rust files"
  fi
fi
