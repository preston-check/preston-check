...
SRC="${SOURCE_DIR:-.}"
login_log=$(grep -rn --include="$SRC_EXT" --max-count=5 'log\..*login\|log\..*signin\|log\..*authenticate\|log\..*CREDENTIALS\|log\..*Login\|log\..*SignIn\|SecurityAuditLogger.logLogin\|security_audit_log' "$SRC" 2>/dev/null | grep -v "test\|Test\|target\|vendor\|_test\.go" | head -3)
...
txn_log=$(grep -rn --include="$SRC_EXT" --max-count=5 'log\..*withdraw\|log\..*deposit\|log\..*transfer\|log\..*payment\|log\..*Withdraw\|log\..*Transfer\|SecurityAuditLogger.logTransaction' "$SRC" 2>/dev/null | grep -v "test\|Test\|target\|vendor\|_test\.go" | head -3)
