...
frameworks: PCI-DSS:4.0:6.2.4, PCI-DSS:4.0:6.5.1, SOC2:TSC-2017:CC8.1, ISO-27001:2022:8.26, OWASP-API:2023:API10, NIST-CSF:2.0:PR.DS-1, CIS-v8:16.9
PRESTON_META
...
if [[ ${_go_files:-0} -gt 0 ]]; then
  _go_hits=$(grep -rn --include="*.go" --exclude-dir=.git --exclude-dir=vendor --exclude-dir=node_modules -E "db\.Query\b|db\.Exec\b|sqlx\.Get|gorm\.Where|os/exec|exec\.Command" "$SRC" 2>/dev/null | grep -vE "_test\.go|/vendor/" || true)
  _go_count=$([[ -n "$_go_hits" ]] && echo "$_go_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_go_count:-0} -gt 0 ]]; then
    record "WARN" "P-08 Input validation (Go)" "$_go_count instance(s) found in Go code" "$(echo "$_go_hits" | head -5)"
  else
    record "PASS" "P-08 Input validation (Go)" "No issues found in Go files"
  fi
fi
...
if [[ ${_rs_files:-0} -gt 0 ]]; then
  _rs_hits=$(grep -rn --include="*.rs" --exclude-dir=.git --exclude-dir=target -E "sqlx::query|diesel::sql_query|std::process::Command|Command::new" "$SRC" 2>/dev/null | grep -vE "#\[cfg\(test\)|/tests?/" || true)
  _rs_count=$([[ -n "$_rs_hits" ]] && echo "$_rs_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_rs_count:-0} -gt 0 ]]; then
    record "WARN" "P-08 Input validation (Rust)" "$_rs_count instance(s) found in Rust code" "$(echo "$_rs_hits" | head -5)"
  else
    record "PASS" "P-08 Input validation (Rust)" "No issues found in Rust files"
  fi
fi
