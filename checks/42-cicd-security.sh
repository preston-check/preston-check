...
SRC="${SOURCE_DIR:-.}"
creds_scripts=$(grep -rn --include="*.sh" --max-count=10 "password\|secret\|api_key\|AWS_ACCESS\|AWS_SECRET" "$SRC" 2>/dev/null | grep -i "deploy\|build\|setup\|install" | grep -v "getenv\|System\|test\|example\|#\|echo\|\${\|export" | head -5)
...
curl_bash=$(grep -rn --include="*.sh" --max-count=5 "curl.*|.*bash\|curl.*|.*sh\|wget.*|.*bash" "$SRC" 2>/dev/null | grep -v "#\|test\|Test" | head -3)
...
ssl_skip=$(grep -rn --include="*.sh" --include="*.java" --max-count=5 "insecure\|ssl.*verify.*false\|no-check-certificate\|trustAllCerts\|ALLOW_ALL" "$SRC" 2>/dev/null | grep -v "test\|Test\|target\|#\|self.*signed\|buildSelfSigned" | head -3)
