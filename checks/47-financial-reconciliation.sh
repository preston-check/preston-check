...
SRC="${SOURCE_DIR:-.}"
reconciliation=$(grep -rn --include="*.java" --include="*.ts" --max-count=5 "reconcil\|Reconcil\|balance.*check\|verifyBalance\|compareBalance\|settlement.*check" "$SRC" 2>/dev/null | grep -v "test\|Test\|target" | head -3)
...
compensation=$(grep -rn --include="*.java" --max-count=5 "compensat\|rollback.*transaction\|saga\|undo.*step\|revert.*payment\|cancel.*transfer" "$SRC" 2>/dev/null | grep -v "test\|Test\|target" | head -3)
