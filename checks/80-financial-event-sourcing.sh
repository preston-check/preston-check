...
if [[ ${_go_files:-0} -gt 0 ]]; then
  _go_hits=$(grep -rn --include="*.go" --exclude-dir=.git --exclude-dir=vendor --exclude-dir=node_modules -E "event.*source|event.*store|event.*log|event.*stream|replay.*event|reconstruct|rehydrat|journal|history.*table|_history|_audit|_log.*table|change.*log|state.*change.*log|as_of|point.*in.*time|temporal|valid_from|valid_to|effective_date|snapshot|lineage|provenance|trace.*id|correlation.*id" "$SRC" 2>/dev/null | grep -vE "_test\.go|/vendor/" || true)
  _go_count=$([[ -n "$_go_hits" ]] && echo "$_go_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_go_count:-0} -gt 0 ]]; then
    record "PASS" "P-80 Financial event sourcing (Go)" "$_go_count pattern(s) found in Go code"
  else
    record "WARN" "P-80 Financial event sourcing (Go)" "No event sourcing/lineage patterns found in Go files"
  fi
fi
...
if [[ ${_rs_files:-0} -gt 0 ]]; then
  _rs_hits=$(grep -rn --include="*.rs" --exclude-dir=.git --exclude-dir=target -E "event.*source|event.*store|event.*log|event.*stream|replay.*event|reconstruct|rehydrat|journal|history.*table|_history|_audit|_log.*table|change.*log|state.*change.*log|as_of|point.*in.*time|temporal|valid_from|valid_to|effective_date|snapshot|lineage|provenance|trace.*id|correlation.*id" "$SRC" 2>/dev/null | grep -vE "#\[cfg\(test\)|/tests?/" || true)
  _rs_count=$([[ -n "$_rs_hits" ]] && echo "$_rs_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_rs_count:-0} -gt 0 ]]; then
    record "PASS" "P-80 Financial event sourcing (Rust)" "$_rs_count pattern(s) found in Rust code"
  else
    record "WARN" "P-80 Financial event sourcing (Rust)" "No event sourcing/lineage patterns found in Rust files"
  fi
fi
