...
if [[ ${_go_files:-0} -gt 0 ]]; then
  _go_hits=$(grep -rn --include="*.go" --exclude-dir=.git --exclude-dir=vendor --exclude-dir=node_modules -E "ctr|currency.*transaction.*report|threshold.*report|reporting.*threshold|large.*cash|sar|suspicious.*activity|flag.*suspicious|report.*suspicious|alert.*compliance|regulat.*report|compliance.*report|audit.*export|transaction.*export" "$SRC" 2>/dev/null | grep -vE "_test\.go|/vendor/" || true)
  _go_count=$([[ -n "$_go_hits" ]] && echo "$_go_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_go_count:-0} -gt 0 ]]; then
    record "PASS" "P-79 Regulatory reporting (Go)" "$_go_count pattern(s) found in Go code"
  else
    record "WARN" "P-79 Regulatory reporting (Go)" "No CTR/SAR/regulatory export patterns found in Go files"
  fi
fi
...
if [[ ${_rs_files:-0} -gt 0 ]]; then
  _rs_hits=$(grep -rn --include="*.rs" --exclude-dir=.git --exclude-dir=target -E "ctr|currency.*transaction.*report|threshold.*report|reporting.*threshold|large.*cash|sar|suspicious.*activity|flag.*suspicious|report.*suspicious|alert.*compliance|regulat.*report|compliance.*report|audit.*export|transaction.*export" "$SRC" 2>/dev/null | grep -vE "#\[cfg\(test\)|/tests?/" || true)
  _rs_count=$([[ -n "$_rs_hits" ]] && echo "$_rs_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_rs_count:-0} -gt 0 ]]; then
    record "PASS" "P-79 Regulatory reporting (Rust)" "$_rs_count pattern(s) found in Rust code"
  else
    record "WARN" "P-79 Regulatory reporting (Rust)" "No CTR/SAR/regulatory export patterns found in Rust files"
  fi
fi
