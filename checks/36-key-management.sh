...
if [[ -z "$key_files" ]]; then
  ...
  record "FAIL" "P-36 Key files in repo" "$count key/cert files in repo (should be in Secrets Manager)" "$(echo "$key_files" | head -10)"
fi
...
if [[ -n "$rotation" ]]; then
  ...
  record "WARN" "P-36 Key rotation" "No key rotation mechanism found" "$(echo "$rotation" | head -10)"
fi
