...
if [[ ${_go_files:-0} -gt 0 ]]; then
  _go_hits=$(grep -rn --include="*.go" --exclude-dir=.git --exclude-dir=vendor --exclude-dir=node_modules -E "training|phishing.*sim|security.*awareness|knowbe4|proofpoint.*training|security.*policy|acceptable.*use|risk.*assessment|vendor.*management|third.*party.*risk" "$SRC" 2>/dev/null | grep -vE "_test\.go|/vendor/" || true)
  _go_count=$([[ -n "$_go_hits" ]] && echo "$_go_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_go_count:-0} -gt 0 ]]; then
    record "PASS" "P-84 Org policies (Go)" "$_go_count pattern(s) found in Go code"
  else
    record "WARN" "P-84 Org policies (Go)" "No security policy/training/risk-assessment references found in Go files"
  fi
fi
...
if [[ ${_rs_files:-0} -gt 0 ]]; then
  _rs_hits=$(grep -rn --include="*.rs" --exclude-dir=.git --exclude-dir=target -E "training|phishing.*sim|security.*awareness|knowbe4|proofpoint.*training|security.*policy|acceptable.*use|risk.*assessment|vendor.*management|third.*party.*risk" "$SRC" 2>/dev/null | grep -vE "#\[cfg\(test\)|/tests?/" || true)
  _rs_count=$([[ -n "$_rs_hits" ]] && echo "$_rs_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_rs_count:-0} -gt 0 ]]; then
    record "PASS" "P-84 Org policies (Rust)" "$_rs_count pattern(s) found in Rust code"
  else
    record "WARN" "P-84 Org policies (Rust)" "No security policy/training/risk-assessment references found in Rust files"
  fi
fi
