...
description: Detects transaction deletion, status rewinds, append-only enforcement, qty zeroing.
category: code-scan
severity: medium
...
frameworks: PCI-DSS:4.0:10.3.2, SOC2:TSC-2017:CC8.1, ISO-27001:2022:8.15, NIST-CSF:2.0:PR.DS-6
PRESTON_META
...
if [[ ${_go_files:-0} -gt 0 ]]; then
  _go_hits=$(grep -rn --include="*.go" --exclude-dir=.git --exclude-dir=vendor --exclude-dir=node_modules -E "append.*only|prevent_delete|immutable|finaliz|settl|commit.*tx|confirmation.*depth" "$SRC" 2>/dev/null | grep -vE "_test\.go|/vendor/" || true)
  _go_count=$([[ -n "$_go_hits" ]] && echo "$_go_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_go_count:-0} -gt 0 ]]; then
    record "PASS" "P-70 Settlement Finality (Go)" "Settlement finality / immutability patterns found in Go code"
  else
    record "WARN" "P-70 Settlement Finality (Go)" "No append-only or settlement finality enforcement found in Go files"
  fi
fi
...
if [[ ${_rs_files:-0} -gt 0 ]]; then
  _rs_hits=$(grep -rn --include="*.rs" --exclude-dir=.git --exclude-dir=target -E "append.*only|prevent_delete|immutable|finaliz|settl|commit_tx|confirmation_depth" "$SRC" 2>/dev/null | grep -vE "#\[cfg\(test\)|/tests?/" || true)
  _rs_count=$([[ -n "$_rs_hits" ]] && echo "$_rs_hits" | wc -l | tr -d ' ' || echo 0)
  if [[ ${_rs_count:-0} -gt 0 ]]; then
    record "PASS" "P-70 Settlement Finality (Rust)" "Settlement finality / immutability patterns found in Rust code"
  else
    record "WARN" "P-70 Settlement Finality (Rust)" "No append-only or settlement finality enforcement found in Rust files"
  fi
fi
