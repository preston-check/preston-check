...
Each check is grounded in a real-world incident or production audit finding: P-700 references storage-collision proxy upgrade incidents, P-701 cites the Wormhole bridge ($320M, Feb 2022), P-708 cites the Cetus integer overflow on Sui (May 2025, ~$220M), P-709 cites Beanstalk Farms ($182M, April 2022). The P-710..P-719 series codifies lessons from production HTLC + atomic-swap audits the maintainers conducted: cross-chain ID binding (HTLC final audit §1.6), preimage non-storage (SC-C2), refund authorization (SC-C1), timelock-bypass predicates (SwapEscrow SC-C1), asymmetric emergency pause (SC-H2), handler rotation timelocks (SC-C3), gas-DoS bounds on held-deposit arrays, abi.encodePacked collision avoidance (SWC-133 / §7.4), bytes-length caps for MPC shard storage (M-6), and block-data randomness misuse.
